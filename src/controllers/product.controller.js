...
export const productPopulate = [
  {
    path: "artist",
    select:
      "username display_name profile_picture banner_picture bio genre role",
  },
  { path: "tracks" },
];
...
const normalizeMerchVariants = (variants) => {
  return variants.map((variant, index) => {
    const stockValue =
      variant.stock ?? variant.stockQuantity ?? variant.stock_quantity;
...
const buildProductQuery = async (queryParams) => {
  ...
  if (text) {
    ...
    if (matchingArtists.length > 0) {
      query.$or.push({
        artist: { $in: matchingArtists.map((item) => item._id) },
      });
    }
  }
...
export const getProductById = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, data: formatPublicProduct(product) });
  } catch (err) {
    next(err);
  }
};
...
export const createSingleProduct = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ success: true, data: formatProduct(populatedProduct) });
  } catch (err) {
    next(err);
  } finally {
...
export const createAlbumProduct = async (req, res, next) => {
  ...
  try {
    const { title, description, price, nameYourPrice, trackTitles } =
      req.body || {};
    ...
    if (trackTitles) {
      try {
        ...
        return res
          .status(400)
          .json({ success: false, message: "Track titles must be valid JSON" });
      }

      if (!Array.isArray(requestedTrackTitles)) {
        return res
          .status(400)
          .json({ success: false, message: "Track titles must be an array" });
      }
    }
    ...
    await session.withTransaction(async () => {
      ...
      const tracksData = audioUploads.map((audioUpload, index) => ({
        artist: req.user.user_Id,
        title:
          requestedTrackTitles[index] ||
          audioFiles[index].originalname
            .replace(/\.[^/.]+$/, "")
            .replace(/[_-]+/g, " ")
            .trim(),
...
export const createMerchProduct = async (req, res, next) => {
  try {
    ...
    if (!merchType) {
      return res
        .status(400)
        .json({ success: false, message: "Merch type is required" });
    }
    ...
    if (!MERCH_TYPES.includes(merchType)) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid merch type" });
    }

    const productStock =
      stock === undefined || stock === "" ? null : Number(stock);

    if (
      productStock !== null &&
      (!Number.isInteger(productStock) || productStock < 0)
    ) {
      return res
        .status(400)
        .json({
    ...
    const merchVariants = normalizeMerchVariants(
      parseJsonArray(variants, "Variants"),
    );
    const weight =
    ...
    if (weight !== null && (!Number.isFinite(weight) || weight < 0)) {
      return res
        .status(400)
        .json({ success: false, message: "Weight must be a valid number" });
    }
    ...
    const product = await Product.create({
      ...
      weightGrams: weight,
      shipsInternationally:
        shipsInternationally === "true" || shipsInternationally === true,
      title,
      slug,
      description,
      price: productPrice,
    ...
    return res
      .status(201)
      .json({ success: true, data: formatProduct(createdProduct) });
  } catch (err) {
    next(err);
  }
};
